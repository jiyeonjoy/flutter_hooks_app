...
import 'package:flutter_hooks_app/star_wars/models.dart';
...
class _PlanetHandler {
  ...
  Future<void> fetchAndDispatch([String? url]) async {
    ...
    try {
      final PlanetPageModel? page = url == null
          ? await _starWarsApi.getPlanets()
          : await _starWarsApi.getPlanets(page: url);
      if (page != null) {
...
class PlanetScreen extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final store = useReducer<AppState, ReduxAction>(
      ...
      initialAction: FetchPlanetPageActionStart(),
    );
